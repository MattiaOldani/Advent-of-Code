...
for i in range(len(grid)):
    visited = dict()
    explore(grid, i, 0, "R", visited)
    energy = max(energy, len(set([(p[0],p[1]) for p in visited.keys()])))
    
    visited = dict()
    explore(grid, i, len(grid[i])-1, "L", visited)
    energy = max(energy, len(set([(p[0],p[1]) for p in visited.keys()])))

    visited = dict()
    explore(grid, 0, i, "D", visited)
    energy = max(energy, len(set([(p[0],p[1]) for p in visited.keys()])))
    
    visited = dict()
    explore(grid, len(grid)-1, i, "U", visited)
    energy = max(energy, len(set([(p[0],p[1]) for p in visited.keys()])))
